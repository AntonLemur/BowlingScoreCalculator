...
unsigned short calculate_score(vector<vector<unsigned short>>& game, int i)
{
	...
	if(game[i-1][game[i-1].size()-1]==10)
		...
		{
			if(game[i-2].size()==1 && (game[i-2][game[i-2].size()-1]%10==0)) //если два фрейма назад был страйк
			    game[i].size()==1?game[i-2][game[i-2].size()-1]+=game[i][game[i].size()-1]:game[i-2][game[i-2].size()-1]+=game[i][0];
		}
